...
ancient_poplymorphisms_counter = 0
i = 0
ancient_recombs = []
while i < len(recombination):	
	if chain[i] == 'NA':
		pass
	elif recombination[i] == 1 and chain[i][ancient_position] == '1':
		ancient_recombs.append(i)
	if chain[i] == 'NA':
		pass
	elif (recombination[i] == 1 or mutation[i] == 1) and chain[i][ancient_position] == '1':
		ancient_poplymorphisms_counter += 1
	i += 1
	

print('\nNumber of R sites in which the ancient DNA has a variant:', len(ancient_recombs))
