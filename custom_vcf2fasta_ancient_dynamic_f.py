...
#exome_extracter.py should be used if just exome positions are wanted and this will produce a full chromosome (for example) even if the vcf file contains only exomes (it is assumed the reference file
#is a full chromosome sine construction of the final sequence and variant positions assumes no gapsa)

import sys, os, subprocess, glob, linecache

'''
usage: custom_vcf2fasta_ancient_f_uf.py
'''
...
if not os.path.exists('filtered_dynamic_coverage'):
    os.makedirs('filtered_dynamic_coverage')
...
def new_fasta_generator_filter(vcf, reference_seq):
	...
	#Determine ancient sample for proper coverage filtering
	if 'denisova' in vcf.lower():
		dp_low = 16
		dp_high = 46
	elif 'altai' in vcf.lower():
		dp_low = 26
		dp_high = 78
	elif 'sidron' in vcf.lower():
		dp_low = 8
		dp_high = 17
	elif 'vindija' in vcf.lower():
		dp_low = 21
		dp_high = 63
	else:
		print 'No ancient DNA with known coverage detected, exiting...'
		sys.exit()
	...
	vcf_file_name = 'filtered_dynamic_coverage/' + vcf_file_name_1
